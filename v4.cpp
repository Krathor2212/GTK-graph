...
public:
    ...
    vector<pair<int, vector<int>>> calculateDominanceAndInfluence() {
...
public:
    MainWindow() {
        ...
        grid.attach(target_ads_entry, 1, 1, 1, 1);
...
void on_dominance_clicked() {
    ...
    for (const auto& entry : result) {
        ...
        // Build the string for connections
        string connectionStr;
        for (int connection : entry.second) {
            connectionStr += to_string(connection) + " ";
        }
        row[columns.col_status] = "Connections: " + Glib::ustring(connectionStr);
    }
}
    ...
    void apply_css() {
        ...
        context->add_provider_for_screen(screen, css_provider, GTK_STYLE_PROVIDER_PRIORITY_USER);
    }
